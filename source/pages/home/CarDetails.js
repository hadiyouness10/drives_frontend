    ...
    return (
      ...
      >
        <Icon name={icon} size={24} style={{ marginRight: '10%' }} />
          <Text style={{ fontSize: '15%', color: "grey" ,marginRight:'10%'}}>{label}</Text>
          {description? (<Text style={{ fontSize: '10%' }}>{value}</Text>):(<Text style={{ fontSize: '20%' }}>{value}</Text>)}
      </View>
    );
  };
    ...
    <View style={{ flex: 1, backgroundColor: "white" }}>
      ...
      >
        ...
        <View style={{ flexDirection: "row", margin: 50 }}>
             ...
             onPress={() =>  {model && number && color ? navigation.push("Edit Car",{insert:false}) : navigation.push("Edit Car",{insert:true})}}
             >
               <Text
                 style={{
                   borderRadius: 5,
